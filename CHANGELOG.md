...
- Long command-line option calculator using `GetLongOpts`, with short-option aliases, validation, and attribution to the contribution proposed in issue #6

### Changed

- Reworked the interfaces guide with a practical polymorphism example and clearer reference-counting guidance
- Clarified that interface GUIDs are optional unless runtime identification or COM interoperability requires them
- Simplified the short-options example and documented the `GetOpts` option table using the official named constants
- Refined the Unix `cwstring` explanation to identify `LC_ALL`, `LC_CTYPE`, and `LANG` as locale inputs

### Fixed

- Documented the required terminating empty entry in a `GetLongOpts` option table
- Corrected the description of `TInterfacedObject`, which is provided by the system RTL rather than the `Classes` unit
